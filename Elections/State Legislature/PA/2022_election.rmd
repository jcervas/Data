...
for (i in offices) {
  ...
  print(i)
    ...
    j_tmp <- elect_collect(tmp, j)
      j_tmp$DEM <- replace_values(two_party(j_tmp$dem,j_tmp$gop))
    tmp_j <- assign(paste0(i,"_",j),j_tmp)
    print(paste0(i,"_",j))
    print(paste0("Votes: ", mean(j_tmp$DEM)))
    print(paste0("Seats: ", sum(1 * (j_tmp$DEM > 0.5)), "D-", sum(1 * (j_tmp$DEM < 0.5)),"R (", per(mean(1 * (j_tmp$DEM > 0.5))),")"))
...
house_seats_votes[n,3] <- sum(1 * (replace_values(two_party(statewide$dem, statewide$gop)) > 0.5))
n <- n+1
}
...
senate_seats_votes[n,3] <- sum(1 * (replace_values(two_party(statewide$dem, statewide$gop)) > 0.5))
n <- n+1
}
