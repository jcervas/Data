...
total_votes_by_party <- function(df) {
  ...
  for (office in offices) {
    ...
    office_df <- subset(df, office %in% office_df$office)
    house <- aggregate(vote ~ office + house_dist + cand_num + last, data = office_df, sum)
    prev_house <- aggregate(vote ~ office + prev_house_dist + cand_num + last, data = office_df, sum)
    sen <- aggregate(vote ~ office + sen_dist + cand_num + last, data = office_df, sum)
    prev_sen <- aggregate(vote ~ office + prev_sen_dist + cand_num + last, data = office_df, sum)
    
    # Convert the aggregates to wide format
    wide_house <- reshape(house, idvar = c("office", "house_dist", "last"), timevar = "cand_num", direction = "wide")
    wide_prev_house <- reshape(prev_house, idvar = c("office", "prev_house_dist", "last"), timevar = "cand_num", direction = "wide")
    wide_sen <- reshape(sen, idvar = c("office", "sen_dist", "last"), timevar = "cand_num", direction = "wide")
    wide_prev_sen <- reshape(prev_sen, idvar = c("office", "prev_sen_dist", "last"), timevar = "cand_num", direction = "wide")
    
    # Merge the wide data frames by office and last name
    office_result <- merge(wide_house, wide_prev_house, by = c("office", "last"), all = TRUE)
    office_result <- merge(office_result, wide_sen, by = c("office", "last"), all = TRUE)
    office_result <- merge(office_result, wide_prev_sen, by = c("office", "last"), all = TRUE)
    
    # Remove unnecessary columns and rename the rest
    office_result <- office_result[, !grepl("NA", colnames(office_result))]
    names(office_result)[-c(1:2)] <- c(paste0("house_", names(wide_house)[-c(1:2)]),
    ...
    if (nrow(result) == 0) {
      ...
      result <- merge(result, office_result, all = TRUE)  # Subsequent offices
    }
  }
