...
n <- length(range_sim)
votes_seats <- rbind(rep(NA, n), rep(NA,n))
rownames(votes_seats) <- c("votes","seats")
colnames(votes_seats) <- range_sim

for (i in 1:n) {
  tmp <- gov_house + range_sim[i]
  seats_tmp <- prop.table(table(1 * (tmp > 0.5)))
  votes_seats[1,i] <- mean(tmp)
  votes_seats[2,i] <- seats_tmp[2]
}
